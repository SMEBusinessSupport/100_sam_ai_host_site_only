...
#                used_ports.append(port_info['8069/tcp'][0]['HostPort'])
            ...
            if port_info and port_info.get('8071/tcp',None):
#                used_ports.append(port_info['8071/tcp'][0]['HostPort'])
...
#            os.mkdir(path)
    ...
    def run_odoo(self,name, db):
        ...
        try:
            ...
            self.dclient.containers.run(image=self.odoo_image,name=name,detach=True,volumes={extra_path:{'bind':self.data_dir,"mode":"rw"}, path: {'bind': "/etc/odoo/", 'mode': 'rw'},self.common_addons:{'bind': "/mnt/extra-addons", 'mode': 'rw'}},ports={8069:port, 8071:lport},tty=True) #Start the container
        ...
        while count < 10:
            try:
                if source_db in sock_db.list():
                    result = True#sock_db.duplicate_database(admin_passwd, source_db, new_db)
                    return result
            except Exception as e:
