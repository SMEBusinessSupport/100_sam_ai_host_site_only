...
class SaasClient(models.Model):
    ...
    server2_id = fields.One2many('saas.client.server','saas_client_id', string="SaaS Server")
    ...
    ip_url = fields.Char("IP URL",help="159.69.19.189:8069")
    ...
    def create_docker_instance(self, domain_name=None):
        ...
        config_path = get_module_resource('sme_saas_base')
    ...
    def login_to_client_instance(self):
        for obj in self:
            ...
            _logger.info("Datbase Name:%s", obj.database_name)
            _logger.info("host_server:%s", host_server)
            _logger.info("db_server:%s", db_server)
            ...
            if response:
                ...
                custom_domains = obj.saas_contract_id.custom_domain_ids
                client_url = obj.client_url
                _logger.info("client_url:%r", obj.client_url)
                for custom_domain in custom_domains:
                    if not custom_domain.revoke_date:
                        client_url = 'http://'+custom_domain.name
                        break
                else:
                    client_url = obj.client_url
                login_url = "{}/saas/login?db={}&login={}&passwd={}".format(client_url, obj.database_name, login, password)
                _logger.info("$$$$$$$$4   %r "%login_url)
                return {
    ...
    def stop_client(self):
        for obj in self:
            host_server, db_server = obj.saas_contract_id.server_id.get_server_details()
            _logger.info("host_server SME:%r", host_server)
            _logger.info("db_server SME:%r", db_server)
            response_flag = containers.action(operation="stop",container_id=obj.container_id,host_server = host_server,db_server = db_server)
            _logger.info("response_flag SME:%r", response_flag)
            if response_flag:
                obj.state = "stopped"
            else:
    ...
    def drop_db(self):
        for obj in self:
            if obj.state == "inactive":
                ...
                response = client.main(obj.database_name, obj.containter_port, host_server, get_module_resource('sme_saas_base'), from_drop_db=True)
    ...
    def drop_container(self):
        for obj in self:
            if obj.state == "inactive":
                host_server, db_server = obj.saas_contract_id.server_id.get_server_details()
                _logger.info("HOST SERER %r   DB SERVER  %r"%(host_server,db_server))
                response = client.main(obj.database_name, obj.containter_port, host_server, get_module_resource('sme_saas_base'), container_id=obj.container_id, db_server=db_server, from_drop_container=True)
