...
class SaasContract(models.Model):
    ...
    on_create_email_template = fields.Many2one('mail.template', default=lambda self: self.env.ref('sme_saas_base.client_credentials_template'), string="Client Creation Email Template")
    ...
    def send_expiry_mail(self):
        for obj in self:
            template = self.env.ref('sme_saas_base.contract_expiry_template')
    ...
    def send_subdomain_email(self):
        for obj in self:
            ...
            template = self.env.ref('sme_saas_base.client_subdomain_template')
            mail_id = template.send_mail(obj.id)
            current_mail = self.env['mail.mail'].browse(mail_id)
            current_mail.send()
    ...
    def add_custom_domain(self, domain=None, is_ssl=False):
        ...
        module_path = get_module_resource('sme_saas_base')
        domain_name = self.domain_name
        if not self.use_separate_domain:
            domain_name += "."+self.saas_domain_url
        response = generate_ssl_custom_domain.main_add(domain_name, domain, is_ssl, module_path)
        if response.get('status'):
            ...
            raise UserError("ER 501: {}".format(error))
