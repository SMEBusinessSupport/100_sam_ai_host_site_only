...
class MailController(http.Controller):
    ...
    def mail_action_view(self, contract_id=None, token=None, partner_id=None, **kwargs):
        ...
        if contract_id and token and partner_id:
            ...
            if contract.exists() and (contract.partner_id.id == int(partner_id)) and (contract.token == token) and (contract.state == 'draft'):
                return request.render('sme_saas_base.subdomain_page', {
    ...
    def domain_set_template(self):
        return request.render('sme_saas_base.redirect_page')
...
class CustomWebsiteSale(WebsiteSale):
    ...
    def values_postprocess(self, order, mode, values, errors, error_msg):
        new_values, errors, error_msg = super(CustomWebsiteSale, self).values_postprocess(order, mode, values, errors, error_msg)
        new_values['instance'] = values.get('instance')
        return new_values, errors, error_msg

    def checkout_values(self, **kw):
        order = request.website.sale_get_order(force_create=1)
        res = super(CustomWebsiteSale, self).checkout_values(**kw)
        order.instance_name = order.partner_id.instance
        request.env['saas.client'].create({'partner_id':order.partner_id.id,
                                           'database_name':order.instance_name
                                           })
        return res
# by malik end